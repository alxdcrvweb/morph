...
export default function MainLayout({ children }: MyHead) {
	...
	useEffect(() => { return () => setPrevPath(router.asPath) }, [router])

	return (
		<>
			<Head>
				<title>{`${routerPathName.slice(0, 1).toUpperCase() + routerPathName.slice(1) || 'Morpheus NFT'}`} | Morpheus</title>
				<meta charSet="UTF-8" />
			...
			<LocomotiveScrollProvider
				...
				onUpdate={(locoScroll: Scroll) => {
					...
					ScrollTrigger.addEventListener("refresh", call);
					ScrollTrigger.refresh();
					if (prevPath.split('/')[1] === 'story'
						|| router.asPath.split('/')[1] === 'story'
						|| prevPath.split('/')[1] === 'terminal'
						|| router.asPath.split('/')[1] === 'terminal') locoScroll.scrollTo(0, { duration: 0, disableLerp: true })
			...
			>
				...
				{(router.asPath === '/sleeping' || router.asPath === '/awake') && <AboutPageLink />}
				{(router.asPath === '/story/1' || router.asPath.split('/')[1] === 'terminal') && <BackLink />}
				<div className={'_decor _decor_1'}>
					<Image src={decor1} alt="" layout='fixed' objectFit='cover' objectPosition={'center'} />
				</div>
				<div className={'_decor _decor_2'} >
					<Image src={decor2} alt="" layout='fixed' objectFit='cover' objectPosition={'center'} />
				</div>
