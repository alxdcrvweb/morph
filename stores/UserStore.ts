...
export class UserStore {
	...
	checkNetwork = async () => {
		...
		if (this?.provider?.chainId === chainId) {
			console.log('this?.provider && this?.provider?.chainId === chainId', this?.provider && this?.provider?.chainId === chainId);
			this.disabledInput = false
			return
		}
		this.disabledInput = true
	...
	paused = async () => {
		...
		const contract = new this.web3!.eth.Contract(CONTRACT_ABI as any, mintAddress);
	...
	mint = async (inputValue: string) => {
		...
		try {
			...
			const currentTokenId = await contract?.methods.currentSuccessfulTokenId().call()
			const iteration = await contract?.methods.mintedOnCurrentIteration(this.provider.selectedAddress, currentTokenId).call()
	...
	getTokens = async () => {
		...
		let arr: any[] = []
		try {
			const currentTokenId = Number(await contract?.methods.currentSuccessfulTokenId().call()) + 5
			console.log('getTokens currentTokenId', currentTokenId);
			for (let tokenId = 1; tokenId <= currentTokenId; tokenId++) {
				const balanceOf = await contract?.methods.balanceOf(this.provider.selectedAddress, tokenId).call()
				...
				if (balanceOf > 0) {
					...
					const metadata: any = await axios.get(uri.replace("ipfs://", "https://ipfs.io/ipfs/"))
